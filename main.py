'''The Lenovo Laptop Service center provides different services to its customers. As and when a
job is arrived, it is allocated to an employee, if no job is assigned. Otherwise it is added to
pending jobs queue. On completion of job by an employee based on elapsed time in minutes, a
job in pending queue is allocated to the corresponding employee.
Assumptions:
...
3. Each job will take a maximum of 1 hr to complete, time needed for completing a job
and time elapsed are given in minutes.'''
...
for emp in company.get_employees():
    print(emp.get_name(), "needs",
          emp.get_allocated_job().get_time_needed() - emp.get_allocated_job().get_time_elapsed(), "more minutes for",
          emp.get_allocated_job().get_name())
    print()
print("Pending Jobs:")
company.get_pending_jobs().display()
